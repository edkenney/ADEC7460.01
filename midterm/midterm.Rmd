...
As seen from the results the ETS model performs the most accurately with a MPE of -3.54% and MAPE of 4.71%. The next closest model, ARIMA, has almost double the error of the ETS model. The ensemble model performs better than the TSLM mode, but not the ARIMA and ETS. Next steps would be to account for bias in the ETS model (ME of -564) and test how the model could apply to smaller, more localized data sets (state or county level).
